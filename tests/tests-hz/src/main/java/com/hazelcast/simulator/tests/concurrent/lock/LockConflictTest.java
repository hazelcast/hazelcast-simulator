   ...
   public class ThreadState extends BaseThreadState {
      ...
      private List<KeyIncrementPair> getLocks(List<KeyIncrementPair> potentialLocks) {
         ...
         for (KeyIncrementPair keyIncrementPair : potentialLocks) {
            try {
               FencedLock lock = getLock(keyIncrementPair);
               try {
                  ...
                  logger.log(Level.WARN, name + ": trying lock=" + keyIncrementPair.key, e);
                  if (throwException) {
                     throw rethrow(e);
                  }
               }
            } catch (Exception e) {
               logger.log(Level.WARN, name + ": getting lock for locking=" + keyIncrementPair.key, e);
               if (throwException) {
                  throw rethrow(e);
               }
            }
      ...
      private void incrementLockedValues(List<KeyIncrementPair> locked) {
         for (KeyIncrementPair keyIncrementPair : locked) {
            try {
               ...
               logger.log(Level.WARN, name + ": updating account=" + keyIncrementPair, e);
               if (throwException) {
                  throw rethrow(e);
               }
            }
            ...
            while (iterator.hasNext()) {
               ...
               try {
                  ...
                  try {
                     ...
                     logger.log(Level.WARN, name + ": unlocking lock =" + keyIncrementPair.key, e);
                     if (throwException) {
                        throw rethrow(e);
                     }
                  }
               } catch (Exception e) {
                  logger.log(Level.WARN, name + ": getting lock for unlocking=" + keyIncrementPair.key, e);
                  if (throwException) {
                     throw rethrow(e);
                  }
               }
      ...
      private FencedLock getLock(KeyIncrementPair keyIncrementPair) {
         return targetInstance.getCPSubsystem().getLock(name + 'l' + keyIncrementPair.key);
      }
   }
