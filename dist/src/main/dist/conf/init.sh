...
#
# NOTE: The script is executed as root, don't use sudo within this script file!
#
...
if [[ "${cloudprovider}" == "aws-ec2" ]]; then
    ...
    if [ ${ver} -lt 319 ]; then
        ...
        echo 'Applying fix: "ethtool -K eth0 sg off"'
...
if [ -d /mnt/ephemeral ] ; then
    if [ -d /mnt/ephemeral/workers ] ; then
        ...
        mkdir /mnt/ephemeral/workers
        chown -R ${user} /mnt/ephemeral/workers/
