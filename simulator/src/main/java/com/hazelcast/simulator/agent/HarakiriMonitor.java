...
import static com.hazelcast.simulator.common.CloudProvider.AWS_EC2;
...
public class HarakiriMonitor extends Thread {
    ...
    public void run() {
        if (agent.cloudProvider != AWS_EC2) {
            LOGGER.info(format("No Harakiri monitor is active: only on %s unused machines will be terminated.", AWS_EC2.desc()));
            return;
        }
