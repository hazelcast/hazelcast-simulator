...
public class Client2JavaSerBenchmark extends HazelcastTest {
    private static final int TRANS_PER_ACCOUNT = 10;
    public int entryCount = 1000;

    private IMap<Client2JavaSerAccountKey, Client2JavaSerAccountValue> accounts;
    private IMap<Client2JavaSerTransKey, Client2JavaSerTransValue> trans;
    ...
    @Prepare(global = true)
    public void prepare() {
        Streamer<Client2JavaSerAccountKey, Client2JavaSerAccountValue> accountStreamer = StreamerFactory.getInstance(accounts);
        Streamer<Client2JavaSerTransKey, Client2JavaSerTransValue> transStreamer = StreamerFactory.getInstance(trans);
        ...
        for (int i = 0; i < entryCount; i++) {
            ...
            accountStreamer.pushEntry(client2JavaSerAccountKey, client2JavaSerValue);
            ...
            for (int j = 0; j < 10; j++) {
                ...
                transStreamer.pushEntry(client2JavaSerTransKey, client2JavaSerTransValue);
            }
        }
        ...
        String query = "CREATE MAPPING IF NOT EXISTS SCB_ACCOUNT"
                ...
                + "        OPTIONS (\n"
                + "                'keyFormat' = 'java',\n"
                + "                'keyJavaClass' = 'com.hazelcast.simulator.tests.map.sql.realprod.client2.Client2JavaSerAccountKey',\n"
                + "                'valueFormat' = 'java',\n"
                + "                'valueJavaClass' = 'com.hazelcast.simulator.tests.map.sql.realprod.client2.Client2JavaSerAccountValue'\n"
                + "        )";
        ...
        query = "CREATE MAPPING IF NOT EXISTS SCB_TRANSACTION"
                ...
                + "                'keyJavaClass' = 'com.hazelcast.simulator.tests.map.sql.realprod.client2.Client2JavaSerAccountKey',\n"
                + "                'valueFormat' = 'java',\n"
                + "                'valueJavaClass' = 'com.hazelcast.simulator.tests.map.sql.realprod.client2.Client2JavaSerTransKey'\n"
                + "        )";
        sqlService.execute(query);
    }
